...
1. 버퍼와 스풀의 차이점은 무엇인가요?
   ...
   ```
   KEYWORDS: 속도차 완화, 프로그램 간 공유, 독립적
   
   버퍼는 일정량의 데이터를 모아 옮김으로써 장치 간 속도차를 완화시켜주는 기술이며, 스풀은 CPU와 입출력장치가 독립적으로 동작하도록 고안된 소프트웨어적 버퍼이다.
...
2. 캐시의 지역성에 대해 설명하시오. ✨
   ...
   캐시는 '메모리의 데이터를 미리 가져와 저장해두는 임시 장소'인데, 이때 미리 가져올 데이터를 정함에 있어서 현재 위치에 가까운 데이터가 멀리 있는 데이터가 사용될 확률이 높다는 것을 뜻하는 것이 공간적 지역성, 가장 가까운 시간에 접근한 데이터가 더 먼 시간에 접근한 데이터보다 사용될 확률이 높은 것이 시간적 지역성이다.
   ```
...
3. 멀티스레드와 멀티 프로세스 환경이 자원공유 관점에서 어떻게 다른가? (K사) ✨
   ...
   ```
   멀티스레드는 프로세스 내에서 메모리를 공유하고, 멀티프로세스는 각각 별개의 메모리를 차지한다 따라서 멀티프로세스는 프로세스 간 통신(IPC)가 필요하다. 멀티프로세스는 한 프로세스에 문제가 발생해도 다른 프로세스로 전달되지 않지만, 멀티스레드의 경우 전체 프로세스에 영향을 미친다.
   ```
...
6. 라운드 로빈 스케줄링을 설명해보세요. ✨
   ...
   ```
   KEYWORDS: 선점형, 순환, 타임 슬라이스
   
   순환 순서 방식이라고도 하는 라운드 로빈 스케줄링은 한 프로세스가 할당받은 시간동안 작업을 완료하지 못하면 준비 큐의 맨 뒤로 가서 다시 자기 차례를 기다리는 선점형 알고리즘 방식이다. 프로세스들이 작업을 완료할 때까지 계속 순환하면서 실행된다. 타임 슬라이스가 너무 큰 경우 FCFS 스케줄링과 다를 바 없어지고, 타임 슬라이스가 너무 작은 경우엔 문맥 교환이 자주 일어나 시스템의 성능이 떨어지기 때문에 타임 슬라이스를 적당한 크기로 설정하는 것이 중요하다.
   ```
...
3. 동기와 비동기의 차이는 무엇인가요? ✨
   ...
   ```
   KEYWORDS: blocking, 반환값
   
   동기는 메소드를 실행시킴과 동시에 반환값이 기대되는 경우이며, 값이 반환되기 전까지 다음 프로세스가 blocking된다. 반면 비동기는 메소드를 실행시킴과 동시에 반환이 기대되지 않으며, 값이 반환되지 않더라도 다음 프로세스가 실행된다. 비동기 통신을 할 경우 데이터를 받는 쪽은 바쁜 대기를 사용하여 데이터가 도착했는지를 직접 확인해야 한다.
   ```
...
5. 세마포어란?
   ...
   ```
   KEYWORDS: 임계구역 해결 방법, 가용한 자원 개수
   
   임계구역 해결 방법 중 하나로, 임계구역에서 나온 프로세스가 동기화 신호를 활용하여 기다리는 프로세스에 신호를 준다. 기다리는 프로세스는 세마포어 큐에 저장되어 있다가 큐에서 나와 임계구역에 진입한다. 가용한 개수를 가진 자원에 대한 접근 제어용으로 사용되고, 세마포어 변수는 그 가용한 자원의 개수로 초기화된다.
   ```
...
4. 스왑이 무엇이고 이를 통한 이점은? (K사)
   ...
   ```
   스왑은 메모리가 모자라서 쫓겨난 프로세스를 모아두는 저장장치의 특별한 공간이다. 스왑을 이용하면 스왑 영역의 크기가 메모리의 크기로 인식되기 때문에 실제 메모리 크기보다 큰 프로그램을 저장하고 실행시킬 수 있다.
   ```
...
6. 페이징과 세그먼테이션 기법의 차이점을 설명해 보세요.
   ...
   (가상 메모리에만 적용되는건지 물리 메모리에도 되는건지 찾아보기)
...
7. 논리적 주소와 물리적 주소의 차이를 설명하시오.
   ...
   논리 주소는 사용자 입장에서 바라본 주소이고 CPU가 생성한 가상의 주소를 말한다. 반면 물리 주소는 하드웨어(메모리 관리자) 입장에서 바라본 주소로 컴퓨터에 꽂힌 렘 메모리의 실제 주소이다.
   ```
...
8. 메모리 오버레이에 대해 알려주세요.
   ...
   프로그램의 크기가 실제 메모리보다 클 때 전체 프로그램을 메모리에 가져오는 대신 적당한 크기로 잘라서 가져오는 방법이다. 실행하는 데 필요한 중요한 모듈만 올려놓고 나머지는 필요할 때마다 메모리에 가져와 사용한다. PC(프로그램 카운터)가 어떤 모듈을 가져오거나 내보낼지 결정한다. 한정된 메모리에서 메모리보다 큰 프로그램의 실행이 가능하도록 해준다. 가상 메모리 시스템의 기본이 되는 개념이다.
   ```
...
1. 가상 메모리란 무엇인가요?
   ...
   크기가 다른 물리 메모리에서 일관되게 프로세스를 실행할 수 있도록 물리 메모리의 크기와 상관없이 프로세스에 큰 메모리 공간을 제공하는 기술이다. 이론적으로는 무한대의 크기를 가지나 실제 크기는 물리 메모리와 스왑 영역의 합이라고 볼 수 있다.
   ```
...
1. 클라이언트/서버 시스템의 구조를 설명해 보세요.
   ...
   ```
   KEYWORDS: 이중 구조, 웹 브라우저, HTTP, HTML, 비용
   ...
   -----
   서비스 제공자 측에서 서버 관리만 잘해준다면 하드웨어의 비용을 줄일 수 있고, 서비스를 변경하기 위해서는 서버만 바꾸면 된다는 매력적인 장점이 있습니다. 그러나 이러한 중앙집중 방식이 갖는 문제점으로는 서버의 과부하가 있습니다.
   ```
...
2. 고가용성이란 무엇인가요?
   ...
   클라이언트/서버 시스템에서 중앙 서버가 고장나 전체 서비스가 중단되는 것을 막기 위해 이중화 작업을 하는 것입니다.
   
   고가용성은 상시 대기, 상호 인계, 컨커런트 액세스 등으로 구성되어 있습니다. 
   ```
