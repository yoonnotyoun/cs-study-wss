...
1. 운영체제란 무엇인가요?
   ...
   - 개인용 컴퓨터의 Windows, Mac OS, 리눅스, 대형 컴퓨터의 유닉스, 리눅스, 윈도우 서버, 스마트폰의 애플의 IOS, 구글의 안드로이드 등 다양한 기기에서 사용되어지고 있음
   ```
...
3. 멀티스레드와 멀티 프로세스 환경이 자원공유 관점에서 어떻게 다른가?? (카카오 본사 기출) (이게 맞나..??)

   ```
   * 멀티 프로세스(=멀티태스크)
   - 멀티 프로세스 환경에서는 IPC 통신을 통해 자원을 공유한다.
   - 멀티 스레드는 전역 변수와 함수 호출을 활용한다.
   ```
...
2. 임계구역에 문제를 해결할 수 있는 조건은 무엇인가요? 
   ...
   - 한정 대기 : 특정 프로세스가 임계구역에 진입하지 못하면 안 된다.
...
6. 세마포어 vs 뮤텍스 차이 (우리는 뮤텍스 안 배운 것 같은데 자주 등장, 근데 뮤텍스가 상호배제래...)
...
2. 교착 상태의 발생 조건에 대해 말해보세요. 
   ...
   ```
   - 시스템 자원 : 어떤 프로세스가 임계구역으로 보호되는 프린터, 스캐너 CD 레코더 등 동시에 같이 사용할 수 없는 시스템 자원을 할당받은 후 양보하지 않은 경우
   - 공유변수 : 한 변수를 할당받은 상태에서 다른 변수를 기다리는 경우
   - 응용 프로그램 : 데이터 베이스는 데이터의 일관성을 유지하기 위해 잠금을 사용 -> 교착상태 발생 가능
   ```
